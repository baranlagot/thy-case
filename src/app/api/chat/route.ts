...
export async function POST(req: Request) {
    ...
    try {
        const result = await openai.chat.completions.create({
            ...
            messages: [
                ...messages,
                { role: "system", content: `You are a helpful bot, helping with information about these food items on the menu: ${menuItems.join(', ')}. Keep the answer as short and minimal as possible and use the messaged language in your answer. Politely try to keep the conversation about the food on the list.` }
            ],
            max_tokens: 300,
        });
